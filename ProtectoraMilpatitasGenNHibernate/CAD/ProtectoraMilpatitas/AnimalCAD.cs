...
public System.Collections.Generic.IList<ProtectoraMilpatitasGenNHibernate.EN.ProtectoraMilpatitas.AnimalEN> BuscarAnimales (string p_nombre, int? p_edad, char p_sexo, string p_centro, ProtectoraMilpatitasGenNHibernate.Enumerated.ProtectoraMilpatitas.EstadoSaludEnum? p_datosMedicos, string p_caracter, Nullable<DateTime> p_fechaLlegada)
{
        ...
        {
                SessionInitializeTransaction ();
                //String sql = @"FROM AnimalEN self where select ani FROM AnimalEN as ani where ((ani.Nombre like CONCAT('%', :p_nombre, '%')) OR :p_nombre is NULL) AND (ani.Edad=:p_edad OR :p_edad=0) AND (ani.Sexo=:p_sexo OR :p_sexo=CHAR(0)) AND ((ani.Centro like CONCAT('%', :p_centro, '%')) OR :p_centro is NULL) AND (ani.DatosMedicos=:p_datosMedicos OR :p_datosMedicos=0) AND ((ani.Caracter like CONCAT('%', :p_caracter, '%')) OR :p_caracter is NULL) AND (DateTime.Compare(ani.FechaLlegada, :p_fechaLlegada)>=0)";
